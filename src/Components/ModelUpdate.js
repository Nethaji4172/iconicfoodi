...
import { qtyItem, qtyChooseItem } from "../Utils/CartSlice";
const ModelUpdate = ({
  item,
  hideModel,
  cartqty,
  ...
  const addConfigProduct = (item, addonsArray) => {
    const find = cartqty.findIndex((fn) => item.id === fn.productItem);
    const arr = [find, addonsArray];
    dispatch(qtyChooseItem(arr));
